...
if (isset($_POST)) {
	//Reteive college name from session storage.
	...
	$marks_ug = mysqli_real_escape_string($conn, $_POST['ug']);
    $marks_pg = mysqli_real_escape_string($conn, $_POST['pg']);
    $marks_ssl= mysqli_real_escape_string($conn, $_POST['ssl']);
    $marks_hsc= mysqli_real_escape_string($conn, $_POST['hsc']);
	//Encrypt Password
	...
	if ($result->num_rows == 0) {
		...
		$sql = "INSERT INTO users(firstname, lastname, email, password, address, city, state, contactno, qualification, stream, passingyear, dob, age, designation, resume, hash, aboutme, skills,college_name,ug,pg,ssc,hsc) VALUES ('$firstname', '$lastname', '$email', '$password', '$address', '$city', '$state', '$contactno', '$qualification', '$stream', '$passingyear', '$dob', '$age', '$designation', '$file', '$hash', '$aboutme', '$skills','$college_name','$marks_ug','$marks_pg','$marks_ssl','$marks_hsc')";
