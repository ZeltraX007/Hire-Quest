...
if(isset($_POST)) {
	...
	$companyname = mysqli_real_escape_string($conn, $_POST['collegename']);
	...
	$sql = "SELECT email FROM college WHERE email='$email'";
	...
	if($result->num_rows == 0) {
		...
		if($uploadOk == false) {
			header("Location: register-college.php");
			exit();
		}

		//sql new registration insert query
		$sql = "INSERT INTO college(name, collegename, country, state, city, contactno, website, email, password, aboutme, logo) VALUES ('$name', '$companyname', '$country', '$state', '$city', '$contactno', '$website', '$email', '$password', '$aboutme', '$file')";
